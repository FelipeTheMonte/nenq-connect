...
// ─── Permissão ──────────────────────────────────────────────────────────────
export const podeAcessarChatGP = cargo =>
  ['Membro de GP', 'Gestão de Pessoas', 'Coordenação Geral'].includes(cargo);
...
// ─── Tela Principal ───────────────────────────────────────────────────────────
export default function ChatGPScreen({ user }) {
  ...
  const IS_COORD = ['Gestão de Pessoas', 'Coordenação Geral'].includes(user.cargo);
  ...
  useEffect(() => {
    getDocs(query(collection(db, 'usuarios'),
      where('cargo', 'in', ['Membro de GP', 'Gestão de Pessoas', 'Coordenação Geral'])))
      ...
      snap => {
        ...
        snap.docs.forEach(d => {
          ...
          if (data.autorId !== user.matricula && !(data.lidas || {})[user.matricula]) {
            batch.update(doc(db, 'chat_gp', d.id), {
  ...
  const enviarTexto = async () => {
    ...
    try {
      await addDoc(collection(db, 'chat_gp'), {
  ...
  const deletarMsg = async id => {
    await updateDoc(doc(db, 'chat_gp', id), {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px 11px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        ...
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 700, color: 'var(--cream)' }}>Chat — GP</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 10px 0', minHeight: 0 }}>
        ...
        {msgs.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px 24px', color: 'var(--muted)' }}>
            ...
            <div style={{ fontSize: 14, lineHeight: 1.7 }}>Seja o primeiro a mandar uma mensagem!<br />Só o time de GP vê este chat.</div>
          </div>
        )}
