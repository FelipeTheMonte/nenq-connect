...
// ─── Permissão ──────────────────────────────────────────────────────────────
export const podeAcessarChat = cargo =>
  ['Membro de Marketing', 'Coordenador de Marketing'].includes(cargo);
...
// ─── Tela Principal ───────────────────────────────────────────────────────────
export default function ChatMarketingScreen({ user }) {
  ...
  const IS_COORD = ['Coordenador de Marketing'].includes(user.cargo);
      ...
      snap => {
        ...
        snap.docs.forEach(d => {
          ...
          if (data.autorId !== user.matricula && !(data.lidas || {})[user.matricula]) {
            batch.update(doc(db, 'chat_marketing', d.id), {
  ...
  const enviarTexto = async () => {
    ...
    try {
      await addDoc(collection(db, 'chat_marketing'), {
  ...
  const deletarMsg = async id => {
    await updateDoc(doc(db, 'chat_marketing', id), {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px 11px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        ...
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 16, fontWeight: 700, color: 'var(--cream)' }}>Chat — Marketing</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 10px 0', minHeight: 0 }}>
        ...
        {msgs.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px 24px', color: 'var(--muted)' }}>
            ...
            <div style={{ fontSize: 14, lineHeight: 1.7 }}>Seja o primeiro a mandar uma mensagem!<br />Só o time de Marketing vê este chat.</div>
          </div>
        )}
