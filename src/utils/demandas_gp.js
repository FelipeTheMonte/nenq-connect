...
export async function processarDemandasExpiradas_demandas_gp() {
  const agora = new Date();
  try {
    const snap = await getDocs(collection(db,'demandas_gp'));
    for (const d of snap.docs.map(d=>{return {id:d.id,...d.data()}})) {
      ...
      if (d.status==='aguardando' && expiracao24 && agora > expiracao24) {
        const mSnap = await getDocs(query(collection(db,'usuarios'), where('cargo','in',['Membro de GP', 'Gestão de Pessoas', 'Coordenação Geral'])));
        ...
        await deleteDoc(doc(db,'demandas_gp',d.id));
      } else if (d.status==='em_progresso' && prazo && agora > prazo) {
        for (const part of (d.participantes||[])) {
          const eSnap = await getDocs(query(collection(db,'envios_demandas_gp'), where('demandaId','==',d.id), where('userId','==',part.userId)));
          if (eSnap.empty) {
